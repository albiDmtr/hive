...
export class ParticleHive extends BaseScriptComponent {
  ...
  @input
  @hint("Prefab 1 to instantiate")
  prefab_1!: ObjectPrefab;

  @input
  @hint("Prefab 2 to instantiate")
  prefab_2!: ObjectPrefab;

  @input
  @hint("Prefab 3 to instantiate")
  prefab_3!: ObjectPrefab;

  @input
  @hint("Prefab 4 to instantiate")
  prefab_4!: ObjectPrefab;
  ...
  private startAnimation(
    ...
    let destinationPosition = originalPosition.add(
      new vec3(
        Math.random() * this.noiseAmplitude * initialRatio,
        Math.random() * this.noiseAmplitude * initialRatio,
        Math.random() * this.noiseAmplitude * initialRatio,
      ),
    );
  ...
  private createPrefabInstance(position: vec3): SceneObject {
    const prefabNo = `prefab_${Math.ceil(Math.random() * 4)}`;

    if (this[prefabNo]) {
      const instance = this[prefabNo].instantiate(this.sceneObject);
