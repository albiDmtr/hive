...
var __selfType = requireType("./AnimateSphere");
    ...
    var CircleAreaInstantiatorTS = _classThis = class extends _classSuper {
        ...
        instantiatePrefabs() {
            ...
            for (let i = 0; i < this.numberOfPrefabs; i++) {
                ...
                delayedEvent.bind(() => { this.animateParticle(pWithPos); });
                // Start with a 2 second delay
                delayedEvent.reset(this.animCycleS / (i / this.numberOfPrefabs));
            }
        }
        animateParticle(particle) {
            const noiseAmplitude = 3;
            let transform = particle.obj.getTransform();
            let startPosition = new vec3(particle.pos.x, particle.pos.y, particle.pos.z);
            let destinationPosition = startPosition.add(new vec3(Math.random() * noiseAmplitude, Math.random() * noiseAmplitude, Math.random() * noiseAmplitude));
            LSTween_1.LSTween.moveFromToLocal(transform, startPosition, destinationPosition, 1500)
...
//# sourceMappingURL=AnimateSphere.js.map