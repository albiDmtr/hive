...
let SphereController = (() => {
    ...
    var SphereController = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.particleHive = this.particleHive;
        ...
        do3DGeneration() {
            ...
            const prompts = ["an airplane", "a running person", "headphones"];
            Snap3D_1.Snap3D.submitAndGetStatus({
                prompt: "a running person",
                format: "glb",
                refine: false,
                use_vertex_color: false,
            })
                .then((submitGetStatusResults) => {
                print(`[INFO] Received status: ${JSON.stringify(submitGetStatusResults)}`);
        ...
        async extractPointcloud(gltfUrl) {
            print("Extracting pointcloud...");
            let request = new Request("https://hive-flax-five.vercel.app/glb-to-pointcloud", {
                method: "POST",
                body: JSON.stringify({
                    url: gltfUrl,
