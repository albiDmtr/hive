...
import {
  Interactable,
  TargetingVisual,
} from "../../../Components/Interaction/Interactable/Interactable";
...
import {
  parseInputState,
  serializeInputState,
} from "../../../Core/SyncInteractionManager/SyncInteractionManager";
...
export class ContainerFrame extends BaseScriptComponent {
  ...
  @hint(
    "Controls the appearance, size, and interaction behavior of the container frame.",
  )
  @input
  @hint(
...
clarity.",
  ...
  @hint(
    "Sets the minimum and maximum dimensions that the container frame can be resized to.",
  )
  /**
  ...
  @hint(
    "Controls how the container frame automatically rotates to face the camera/user.",
  )
  /**
...
unless yAlways is also enabled.",
  ...
  @hint(
    "Controls how the container snaps to other containers or world features when moved.",
  )
  /**
...
moved.",
  ...
  @hint(
    "Controls whether the container automatically follows the user's view when moving around.",
  )
  /**
  ...
  @hint(
    "Controls whether a close button is displayed in the corner of the container.",
  )
  /**
...
- 2: Ray",
  ...
  @widget(
    new ComboBoxWidget([
      ...
      new ComboBoxItem("Cursor", 1),
      new ComboBoxItem("Ray", 2),
    ]),
  )
...
If set to true, the Container's position will be synced whenever a new user joins the same Connected Lenses session.",
  ...
  private readonly syncEntity = this.isSynced
  ...
  onAwake() {
     ...
     */
    this.interactable.targetingMode =
      TargetingMode.Indirect | TargetingMode.Direct;
    ...
    this.unSubscribeList.push(
      this.interactable.onTriggerStart((e: InteractorEvent) => {
        if (e.interactor.targetHitInfo === null) {
          throw new Error(
            `targetHitInfo of the triggering Interactor is an invalid value.`,
          );
        }

        const initialHit = e.interactor.targetHitInfo.localHitPosition;
        const worldHitPosition =
          e.interactor.targetHitInfo.interactable.sceneObject
            .getTransform()
            .getWorldTransform()
        ...
        if (this.billboardComponent !== null) {
          this.billboardComponent.setPivot(
            this.billboardComponent.targetTransform
              .getInvertedWorldTransform()
              .multiplyPoint(worldHitPosition),
            e.interactor,
          );
        }
    ...
    this.unSubscribeList.push(
      this.interactable.onTriggerUpdate((event: InteractorEvent) => {
        if (event.interactor.targetHitInfo && this.inputHandler.state.scaling) {
          ...
          this._interactionPlane.planeSize = this.totalInnerSize.add(
    ...
    this._interactionPlane = this.sceneObject.createComponent(
      InteractionPlane.getTypeName(),
    );
    this._interactionPlane.planeSize = this.totalInnerSize.add(
      vec2.one().uniformScale(this.border * 2),
    );
    this._interactionPlane.proximityDistance =
  ...
  private setupConnectionCallbacks(): void {
    if (
      ...
      const locationTransform = this.syncEntity.currentStore.getMat4(
        CONTAINER_TRANSFORM_VALUE_KEY,
      );
      const worldTransform =
        this.syncKitBridge.locationTransformToWorldTransform(locationTransform);

      const innerSize = this.syncEntity.currentStore.getVec2(
        CONTAINER_INNER_SIZE_VALUE_KEY,
      );

      const inputStateSerialized = this.syncEntity.currentStore.getString(
        CONTAINER_INPUT_STATE_VALUE_KEY,
      );
      ...
      const locationTransform =
        this.syncKitBridge.worldTransformToLocationTransform(
          this.parentTransform.getWorldTransform(),
        );

      const inputStateSerialized = serializeInputState(this.inputState);

      this.syncEntity.currentStore.putVec2(
        CONTAINER_INNER_SIZE_VALUE_KEY,
        this.innerSize,
      );
      this.syncEntity.currentStore.putMat4(
        CONTAINER_TRANSFORM_VALUE_KEY,
        locationTransform,
      );
      ...
      this.syncEntity.currentStore.putString(
        CONTAINER_INPUT_STATE_VALUE_KEY,
        JSON.stringify(inputStateSerialized),
      );
    }
  }
  ...
  private update = () => {
    ...
    if (this.inputHandler.state.translating) {
      ...
      if (
        ...
        (!(this.isFollowing && isConnection) || this.xAlways || this.yAlways)
      ) {
        if (this.translatingLastFrame) {
    ...
    const borderRecalculationNeeded =
      this.currentBorder !== this.material.mainPass.frameMargin;
    const constantPaddingChanged = !this.constantPadding.equal(
      this.lastConstantPadding,
    );
    ...
    if (this.syncEntity !== null && this.syncEntity.isSetupFinished) {
      const currentConnectionId = this.syncEntity.currentStore.getString(
        CONTAINER_CONNECTION_VALUE_KEY,
      );
      ...
      if (
        ...
        (this.interactable.triggeringInteractor & InteractorInputType.Sync) ===
          0
      ) {
        this.syncEntity.currentStore.putString(
          CONTAINER_CONNECTION_VALUE_KEY,
      ...
      if (
        ...
        const worldTransform =
          this.syncKitBridge.locationTransformToWorldTransform(
            locationTransform,
          );
  ...
  private scaleFrame = (isTrueScaleUpdate: boolean = true) => {
    ...
    const shouldInvokeScalingUpdate =
      !this.forcePreserveScale && isTrueScaleUpdate;
